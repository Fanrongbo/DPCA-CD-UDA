...
from model.Siamese_diffDA import FCSiamDiffDA
...
if __name__ == '__main__':
    ...
    opt.model_type = 'MemoryAdaptNet'
    cfg.DA.NUM_DOMAINS_BN = 1
    ttest = False
    saveLast = False
    name = opt.dset + 'CDPretrain2/' + opt.model_type
    ...
    if opt.load_pretrain:
        #
        # saveroot = './log/CD_DA_buildingCDBase/BSANet/20231212-13_28_LEVIR_CDPatch/'
        # save_path = saveroot + '/savemodel/_14_acc-0.9891_chgAcc-0.8720_unchgAcc-0.9942.pth'
        saveroot = './log/CD_DA_buildingCDBase/BSANet/20231213-17_19_GDbuilding/'
        save_path = saveroot + '/savemodel/_11_acc-0.9886_chgAcc-0.8335_unchgAcc-0.9952.pth'
        # opt.num_epochs = 20
        # Nepoch=1
    else:
        saveroot = None
    ...
    for epoch in range(start_epoch, opt.num_epochs + opt.num_decay_epochs + 1):
        ...
        for i in tbar:
            ...
            if (epoch - 1) * train_data_len + i <= 3000:
                memory, pred1, pred2 = net.forward(x1=ST1, x2=ST2, mode='TRAIN', target=labelS, lr=LEARNING_RATE,
                                           i_iter=(epoch - 1) * train_data_len + i)
                # loss_seg_stage1 = loss_calc(pred1, labels, args.gpu)
                loss_seg_stage1 = tool.loss(pred1, labelS.long())
                ...
                loss_seg2 = tool.loss(pred2, labelS.long())
                # loss_seg2.backward()
                loss = loss_seg2 + loss_seg_stage1
                loss.backward()

            memory, pred_target1, pred_target2 = net.forward(x1=TT1, x2=TT2, mode='TARGET', target=labelT, lr=LEARNING_RATE,
                                                     i_iter=(epoch - 1) * train_data_len + i)
            D_out = model_D(F.softmax(pred_target1, dim=1))
            loss_adv_target = bce_loss(D_out,
                                       Variable(torch.FloatTensor(D_out.data.size()).fill_(source_label)).to(DEVICE))
            ...
            loss_D = bce_loss(D_out,
                              Variable(torch.FloatTensor(D_out.data.size()).fill_(target_label)).to(DEVICE))
            loss_D.backward()
            loss_D_value += loss_D.item()
            optimizer.step()
            optimizer_D.step()

            cd_predSo = torch.argmax(F.softmax(pred, dim=1), dim=1)  #
            softmaxpreT = F.softmax(pred_target, dim=1)
            ...
            lossT = loss_D_value + TCELoss.item() + loss.item()
            ...
            Score = {'LossT': lossT, 'SCET': loss.item(), 'DGANT': loss_D_value, 'TCET': TCELoss.item() }
            record['LossT'] += lossT
        ...
        with torch.no_grad():
            ...
            for i in tbar:
                ...
                m, outputs1, outputs2 = net(x1=data_T1_val, x2=data_T2_val, mode='TEST', target=label, lr=LEARNING_RATE,
                                            i_iter=(epoch - 1) * train_data_len + i)
                ...
                val_pred = outputs2.detach().max(dim=1)[1].cpu().numpy()
                # cd_val_pred = net.forward(data_T1_val, data_T2_val)

                TCELoss = tool.loss(outputs1, label.long())
        ...
        exel_out = opt.phase, epoch, lossAvg, CES_lossAvg, DGAN_LossAvg, CET_lossAvg,0, IterValScore['acc'], \
        ...
        if saveLast:
            if epoch == opt.num_epochs:
                ...
                tool.save_ckptGANCenter(network=[net,None,None],Center=[None,0,0], optimizer=optimizer, save_str=save_str)
            ...
            tool.save_ckptGANCenter(network=[net, None, None], Center=[None,0,0], optimizer=optimizer,
                                    save_str=save_str)
            ...
            for kk in range(len(cfg.TRAINLOG.DATA_NAMES)):
                ...
                with torch.no_grad():
                    ...
                    for i in tbar:
                        ...
                        m, outputs1, outputs2 = net(x1=data_T1_test, x2=data_T2_test, mode='TEST', target=label,
                                                    lr=LEARNING_RATE,
                                                    i_iter=(epoch - 1) * train_data_len + i)
                        preds1 = (outputs1 + outputs2).detach().max(dim=1)[1].cpu().numpy()
                        target_pred = outputs2.detach().max(dim=1)[1].cpu().numpy()
                        # cd_target_pred = net.forward(data_T1_val, data_T2_val)
                    ...
                    exel_out = 'T-' + cfg.TRAINLOG.DATA_NAMES[kk][
                        0], epoch, lossAvg, CES_lossAvg, DGAN_LossAvg, CET_lossAvg,0, IterTargetScore['acc'], \
