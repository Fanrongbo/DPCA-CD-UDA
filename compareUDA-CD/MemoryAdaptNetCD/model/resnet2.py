...
class ResNet_diff(nn.Module):

    def __init__(self, block, layers, out_stride, mult_grid):
        self.inplanes = 64
        super(ResNet_diff, self).__init__()
        if out_stride == 8:
            stride = [2, 1, 1]
        elif out_stride == 16:
            stride = [2, 2, 1]
...
def resnet101_diff(out_stride=32, mult_grid=False):
    """Constructs a ResNet-101 model."""
    model = ResNet_diff(Bottleneck, [3, 4, 23, 3], out_stride, mult_grid)
    return model
