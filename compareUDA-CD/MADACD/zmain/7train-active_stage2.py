...
class Class_Features:
    ...
    def calculate_min_mse(self, single_image_objective_vectors,centroids):
        loss = []
...
if __name__ == '__main__':
    ...
    saveLast=True
    name = opt.dset + 'CDDA/' + opt.model_type + 'stage2'
    opt.LChannel=True
    ...
    # model_D2.to(DEVICE)
    CAU_full = torch.load('/data/project_frb/DA/DACDCompare/MADACD/zmain/output/G-L/anchors/Target_cluster_centroids_full_10.pkl')
    print(CAU_full.shape)
    ...
    for epoch in range(start_epoch, opt.num_epochs + opt.num_decay_epochs + 1):
        ...
        for i in tbar:
            ...
            CELossAC = tool.loss(cd_predT, labelT.long())#####AC-CE
            ...
            CELoss = tool.loss(cd_predS, labelS.long())  # source-CE
            # net.eval()
            # loss = loss_seg1
            # print(cd_predS.shape, labelS.long().shape)
            source_vectors, source_ids = class_features.calculate_mean_vector(defeatSDA, cd_predS, labelS)
            source_objective_vectors = torch.zeros([2, 32]).to(DEVICE)
            for t in range(len(source_ids)):
                source_objective_vectors[source_ids[t]] = source_vectors[t].squeeze()
            # target map
            target_outputs_softmax = F.softmax(cd_predT, dim=1)
            target_outputs_argmax = target_outputs_softmax.argmax(dim=1, keepdim=True)
            target_vectors, target_ids = class_features.calculate_mean_vector(defeatTDA, cd_predT,
                                                                    target_outputs_argmax.float())
            target_objective_vectors = torch.zeros([2, 32]).cuda()
            for t in range(len(target_ids)):
                target_objective_vectors[target_ids[t]] = target_vectors[t].squeeze()
            loss_L2_source_cls, _ = class_features.calculate_min_mse(source_objective_vectors,CAU_full)
            loss_L2_target_cls, min_index = class_features.calculate_min_mse(target_objective_vectors,CAU_full)
            CAU_full[min_index] = CAU_full[min_index] * 0.999 + 0.001 * target_objective_vectors.detach().cpu().numpy()
            loss_L2_cls = 0.7 * loss_L2_target_cls
            # print(cd_predT.shape, target_outputs_argmax.long().shape)
            # loss_CTS = tool.loss(cd_predT, target_outputs_argmax)#pseudo
            (CELossAC+torch.sum(loss_L2_cls)).backward()
