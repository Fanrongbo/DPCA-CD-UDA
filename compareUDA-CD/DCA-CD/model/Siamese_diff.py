...
class FCSiamDiff(nn.Module):
    ...
    def forward(self, pre_data, post_data):
        ...
        # up_feature_7Out = self.conv_block_9(up_feature_7)
        # up_feature_7Out = F.interpolate(up_feature_7Out, (H, W), mode='bilinear', align_corners=True)
        # output_featurePatch=self.avgpool(G-L)

        return output_feature,output_feature,concat_feature_8
...
class genpatchwithMaskEntropy(nn.Module):
    ...
    def forward(self, infeat, labelTpesudo,labelT,FeatureDA, device=None,k=2):
        # G-L = F.softmax(infeat[:,:,self.kernel:-self.kernel,self.kernel:-self.kernel], dim=1)
        ...
        # print(output_entropyPatch.shape,G-L.shape)#[20, 2, 256, 256]
        output_featurePatch = self.avgpool(output)
            ...
            for kk in range(k):
                ...
                for i in range(output.shape[0]):
                    ...
                    oy=torch.tensor([torch.maximum(self.zero,py*self.stride-self.padding),
                                    ...
                                    dtype=torch.int32).to(device)
                    # outputOut=G-L[i,c,oy[0]:oy[1]+1,ox[0]:ox[1]+1].unsqueeze(0)
...
class genpatch(nn.Module):
    ...
    def forward(self, infeat, labelTpesudo,labelT,FeatureDA, device=None,k=2):
        ...
        output=F.softmax(infeat)
        output_featurePatch = self.avgpool(output)
        # output_featurePatch=F.conv2d(G-L,self.kernelarray,stride=self.stride,padding=self.padding)
