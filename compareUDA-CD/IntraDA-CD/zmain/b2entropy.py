...
def colorize_save(output_pt_tensor, colormap, num_clc, name):
    ...
    mask_Img.save('./outnew/GD-WH/color_masks/%s' % (name))
    mask_color.save('./outnew/GD-WH/color_masks/%s_color.png' % (name.split('.')[0]))
...
def cluster_subdomain(entropy_list, lambda1):
    ...
    with open('./outnew/GD-WH/easy_split.txt', 'w+') as f:
        for item in easy_split:
            # print('item',item)
            f.write('%s\n' % item)

    with open('./outnew/GD-WH/hard_split.txt', 'w+') as f:
