...
if __name__ == '__main__':
    ...
    opt.num_epochs = 30
    opt.easyspilt='/data/project_frb/DA/DACDCompare/CDDA2/IntraDA-CD/zmain/outnew/GZ-LE/easy_split.txt'
    opt.hardspilt='/data/project_frb/DA/DACDCompare/CDDA2/IntraDA-CD/zmain/outnew/GZ-LE/hard_split.txt'
    opt.batch_size = 14
    ...
    ttest = False
    saveLast = False
    name = opt.dset + 'CDPretrain2/' + opt.model_type + 'DAhard'
    ...
    if opt.load_pretrain:
        #
        # saveroot = '/data/project_frb/DA/DACDCompare/CDDA2/IntraDA-CD/zmain/log/CD_DA_buildingCDPretrain2/2-3CenterAttenadvent/20231216-03_44_G-WH/'  #
        # save_path = saveroot + '/savemodel/_11_acc-0.9670_chgAcc-0.8178_unchgAcc-0.9734.pth'  #
        saveroot = '/data/project_frb/DA/DACDCompare/CDDA2/IntraDA-CD/zmain/log/CD_DA_buildingCDPretrain2/2-3CenterAttenadvent/20231215-22_10_G-LE/'  #
        save_path = saveroot + '/savemodel/_11_acc-0.8922_chgAcc-0.4269_unchgAcc-0.9556.pth'  #
        opt.num_epochs = 30
        Nepoch=1
    else:
        saveroot = None
    ...
    net.to(DEVICE)
    from advent.domain_adaptation.config import cfg as cfgadven
    ...
    optimizer = torch.optim.SGD(net.parameters(cfgadven.TRAIN.LEARNING_RATE),
                                lr=cfgadven.TRAIN.LEARNING_RATE,
                                momentum=cfgadven.TRAIN.MOMENTUM,
                                weight_decay=cfgadven.TRAIN.WEIGHT_DECAY)
    d_main = get_fc_discriminator(num_classes=2)
    d_main.to(DEVICE)
    # optimizer_d_aux = optim.Adam(d_aux.parameters(), lr=cfgadven.TRAIN.LEARNING_RATE_D,
    #                              betas=(0.9, 0.99))
    optimizer_d_main = optim.Adam(d_main.parameters(), lr=cfgadven.TRAIN.LEARNING_RATE_D,
                                  betas=(0.9, 0.99))
    ...
    for epoch in range(start_epoch, opt.num_epochs + opt.num_decay_epochs + 1):
        ...
        for i in tbar:
            ...
            adjust_learning_rate(optimizer, epoch, cfgadven)
            # adjust_learning_rate_discriminator(optimizer_d_aux, i, cfgadven)
            adjust_learning_rate_discriminator(optimizer_d_main, epoch, cfgadven)
            ...
            # d_out_main = d_main(defeatTDA)
            d_out_main = d_main(prob_2_entropy(F.softmax(cd_predT, dim=1)))
            loss_adv_trg_main = bce_loss(d_out_main, source_label)

            GANloss = cfgadven.TRAIN.LAMBDA_ADV_MAIN * loss_adv_trg_main
            GANloss.backward()
            optimizer.step()
            ...
            Score = {'CEL': CELoss, 'DALoss': loss_d_mainT.item() + loss_d_mainS.item(), 'DiceL': GANloss.item()}
        ...
        if saveLast:
            if epoch == opt.num_epochs:
                ...
                tool.save_ckptGANCenter(network=[net,d_main,d_main],Center=[None,None,None], optimizer=optimizer, save_str=save_str)
            ...
            tool.save_ckptGANCenter(network=[net, d_main, d_main], Center=[None,None,None], optimizer=optimizer,
                                    save_str=save_str)
