...
class Conv2dLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, pad_type='zero',
                 activation='lrelu', norm='none', sn=True, groups=1):
        ...
        if norm == 'bn':
            ...
            self.norm = nn.GroupNorm(out_channels / 4, out_channels, eps=1e-05, affine=True)
        elif norm == 'none':
            self.norm = None
        else:
        ...
        if sn:
            self.conv2d = SpectralNorm(
                nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=0, dilation=dilation, bias=False))
        else:
...
class ResConv2dLayer(nn.Module):
    def __init__(self, channels, kernel_size=3, stride=1, padding=1, dilation=1, pad_type='zero', activation='lrelu',
                 norm='none', sn=True):
        ...
        self.conv2d = nn.Sequential(
            ...
            Conv2dLayer(channels, channels, kernel_size, stride, padding, dilation, pad_type, activation='none',
                        norm='none', sn=sn)
        )
...
class ConvLSTM2d(nn.Module):
    def __init__(self, input_size, hidden_size, kernel_size=3):
        ...
        self.Gates = nn.Conv2d(input_size + hidden_size, 4 * hidden_size, kernel_size=self.kernel_size, stride=1,
