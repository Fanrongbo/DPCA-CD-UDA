...
class ConfuseMatrixMeter():
    ...
    def get_scoresT(self):
        ...
        core_dict = {'accT': scores_dict['acc'], 'chgT': scores_dict['chgAcc'],
                     'unchgT': scores_dict['unchgAcc'], 'mF1T': scores_dict['fm1'],'miouT':scores_dict['miou']}
        message='T:'
...
def cm2F1(confusion_matrix):
    ...
    score_dict = {'acc': acc, 'chgAcc': chgAcc, 'unchgAcc': unchgAcc,
                  'tp': int(tp[1]), 'fn': int(fn[1]), 'fp': int(fp[1]), 'tn': int(tn[1]),'fm1':mean_F1,'recall':recallM,'precision':precisionM,'miou':mean_iu}
    return score_dict
