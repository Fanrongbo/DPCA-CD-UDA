...
def entropyplot(name='Source',inputimg=None,outputimg=None):
    ...
    title = ['%s T1' % name, '%s T2' % name, 'Predict Entropy']
    print('title',title)
    N = len(title)
    # image = [img1, img2, labelT, Pro[:,1,:,:], outputEntropy]
    image = [img1, img2, outputEntropy]
    ...
    fig, axes = plt.subplots(drawBatch, N, figsize=(12, 18))  # 假设每张子图为256x256像素，所以整体尺寸为10.24x7.68英寸
    dN=0
    for ax, img in zip(axes.ravel(), images):
        ...
        dN += 1
    plt.figure(num=1)
    plt.savefig('./log/experiment/entropy/G-L-%s2/%d.png' % (name,epoch), dpi=100, bbox_inches='tight',
                pad_inches=0)
...
if __name__ == '__main__':
    ...
    if opt.load_pretrain:
        # saveroot = './log/CD_DA_buildingCDPretrain2/2-3CenterAttenExperimnet/20231203-10_37_L-GZ-FCSiameseSource/'
        # save_path = saveroot + '/savemodel/_31_acc-0.9907_chgAcc-0.8515_unchgAcc-0.9967.pth'
        saveroot = './log/CD_DA_buildingCDPretrain2/2-3CenterAttenExperimnet/20231203-10_37_G-LE-FCSiameseSource/'
        save_path = saveroot + '/savemodel/_31_acc-0.9033_chgAcc-0.5937_unchgAcc-0.9454.pth'
        # saveroot = './log/CD_DA_buildingCDPretrain2/2-3CenterAttenExperimnet/20231203-00_18_W-GD-FCSiameseSource/'
        # save_path = saveroot + '/savemodel/_31_acc-0.9820_chgAcc-0.8976_unchgAcc-0.9855.pth'#
        # saveroot = './log/CD_DA_buildingCDPretrain2/2-3CenterAttenExperimnet/20231202-22_28_G-WH-FCSiameseSource/'
        # save_path = saveroot + '/savemodel/_31_acc-0.9708_chgAcc-0.8874_unchgAcc-0.9743.pth'
    else:
        saveroot = None
        save_path = None
    ...
    time_now = time.strftime(
        "%Y%m%d-%H_%M_" + cfg.TRAINLOG.DATA_NAMES[opt.s][0] + '-' + cfg.TRAINLOG.DATA_NAMES[opt.t][:2] + '-FCSiameseSourceTest',
        time.localtime())
