...
if __name__ == '__main__':
    ...
    ttest = True
    saveLast = False
    name = opt.dset + 'CDPretrain2/' + opt.model_type + 'EntropyMidGAN-Dist'
    ...
    if opt.load_pretrain:
        #
        saveroot = './log/CD_DA_buildingCDPretrain2/2-3CenterAttenEntropyMidGAN-itermultiCenter-New/20231130-10_34_G-LE-pasteMean-iterAdjust_Th015aug/'
        save_path = saveroot + '/savemodel/_10_acc-0.8812_chgAcc-0.6573_unchgAcc-0.9117.pth'
        opt.num_epochs = 20
        Nepoch=1
    else:
        saveroot = None
    ...
    time_now = time.strftime(
        "%Y%m%d-%H_%M_" + cfg.TRAINLOG.DATA_NAMES[opt.s][0] + '-' + cfg.TRAINLOG.DATA_NAMES[opt.t][:2] + '-paste3-iter-NoAdjust_Th015aug-040',
        time.localtime())
    ...
    for epoch in range(start_epoch, opt.num_epochs + opt.num_decay_epochs + 1):
        ...
        for i in tbar:
            ...
            if ((epoch > 2 and opt.load_pretrain) or(epoch > 5 and not opt.load_pretrain)) and True:
            # if True:
                for cc in range(labelS.size(0)):
                    if labelRatio[cc] > 0.1 and labelRatio[cc] < 0.5 and PseudoRatio[cc] < 0.01 and EntropyM[cc] < entropyT:
                        ...
                        TFlag = True
                        BInput = torch.cat(
                            [TT1[cc].unsqueeze(0), TT1[cc].unsqueeze(0)], dim=0)
                        STInput = torch.cat(
                            [ST1[cc].unsqueeze(0), ST2[cc].unsqueeze(0)], dim=0)
                        corrected_imgBS = retinex_synthesisModel.forward(BInput, STInput)
        ...
        position = int(0.40 * i * opt.batch_size)
        ...
        exel_out = opt.phase, epoch, lossAvg, CES_lossAvg, DGAN_LossAvg, CET_lossAvg, IterScore['acc'], IterScore[
            'unchgAcc'], \
                   ...
                   float(entropyThrOut['entropymeanValue'].cpu().numpy()), float(entropyThrOut['entropyunchgValue'].cpu().numpy()), \
                   float(entropyThrOut['entropychgValue'].cpu().numpy()), float(entropyT.cpu().numpy()),
